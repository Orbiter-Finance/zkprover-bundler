...
pub async fn receive_tx(tx: Transaction) -> anyhow::Result<H256, anyhow::Error> {
    ...
    if one.is_none() {
        let pool_tx = PoolTx {
            ...
            tx_from,
            tx_hash,
            create_at: DateTime::from(SystemTime::now()),
            status: 1,
        };
...
pub async fn batch_received_txs() -> anyhow::Result<usize, anyhow::Error> {
    ...
    let find_options = FindOptions::builder().limit(BATCH_TX_TOTAL as i64).build();
    ...
    if tx_hash_list.len() >= BATCH_TX_TOTAL {
        info!(
            "to_bson(&tx_hash_list).unwrap(): {:#?}",
            to_bson(&tx_hash_list).unwrap()
        );
        // Lock txs
        co_pool_tx
            .update_many(doc! {"tx_hash": {"$in": {}}}, doc! {"status": 2}, None)
            .await?;
